...
const ModelsHouses = (props) => {
	...
	return (
		<div className={`models-houses__c f-grid cols-${COLUMNS} rows-${ROWS}`}>

			<div className="f-grid-group">
			{ createArrayGroups(COLUMNS, houses).map((houseRow) =>

				<div className="f-grid-row">
				{ houseRow.map((house, houseIndex) =>

					<div 
						className="models-houses__item f-grid-item "
						key={houseIndex+house.title} 
					>
							...
							<div className="models-houses__item-content">
								<div className="models-houses__item-inner-content">
									<div className="models-houses__item-tags">
										<span className={`models-houses__item-status ${house.status?.toLowerCase().includes('preorder') ? 'preorder' :''}`}>{house.status}</span>
