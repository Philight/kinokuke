...
const ImageCarousel = forwardRef((props, ref) => {
	let { 
		className, images, showOverlay, showViewBox, showStartBtn, startBtnText, bidirection,
		heading, text, btnText, btnSrc, isHTML, columns, rows 
	} = props;
	...
	return (
		<section className={`image-carousel__c ${className} f-grid rows-${ROWS} cols-${COLUMNS}`}
			ref={ref}
		>
			{ showStartBtn && <a className={`image-carousel__start-btn ${!isInitial ? 'disabled' :''}`} onClick={startSlide}>
				{startBtnText}
			</a> }

			{ showViewBox && !isInitial && <div className={`image-carousel__view-box absolute-fill`}>
				...
				<a href="/process" rel="noopener noreferrer" className={`image-carousel__view-box-link absolute-fill`} />
			</div> }

			<div className={`image-carousel__images-view carousel-view`}>
				<div className={`image-carousel__images-container carousel-slider`} style={{ transform: `translateX(${isInitial ? '0' : slide*-moveBy+moveOffset}px)` }}>
					{ images.map((image, index) => 
						<div className={`image-carousel__image-wrapper f-center f-grid-item`}>
							<img 
								className={`image-carousel__image`}
								src={image.src} 
								ref={el => imgRefs.current[index] = el} 
								onLoad={() => index==0 && updateImageData(index, {
							...
							{ image.url && <Link to={image.url} className="absolute-fill">
			...
			<div className="image-carousel__content">
				{!isHTML && heading && <h2 className="image-carousel__content-title">{Functions.strReplaceNewline(heading)}</h2>}
				{isHTML && heading && <h2 className="image-carousel__content-title" dangerouslySetInnerHTML={{ __html: heading }} />}			
			...
			<Icon 
				icon="chevron-right" className="nav-arrow right flying highlight primary" 
				onClick={handleSlide}
			/>
		</section>
	)
