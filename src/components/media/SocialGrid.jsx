		...
		<section className={`social-grid__c f-grid cols-${COLUMNS} rows-${ROWS}`}>
			...
			<div className={`carousel-view social-grid__posts`}>
				...
				{ createArrayGroups((ROWS*COLUMNS), posts).map((gridGroup, groupIndex) => (
					<div className="f-grid-group carousel-group social-grid__posts-group" style={{ transform: `translateX(-${activeIndex * 100}%)` }}>
						
						{ createArrayGroups(COLUMNS, gridGroup).map((gridRow, postIndex) => (
							<div className="f-grid-row">
								{ gridRow.map((post, postIndex) => (
									<div className="f-grid-item social-grid__post">
										<Image src={post.imageSrc} />
					        		<a href={INSTAGRAM_URL} target="_blank">
					        			<div className={`social-grid__post-content f-col f-center absolute-fill ${postIndex%2 && contentShown ? 'shown' :''}`}>
