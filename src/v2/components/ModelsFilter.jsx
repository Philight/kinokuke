...
const ModelsFilter = (props) => {
  ...
  const leftArrowURL =
    'https://ik.imagekit.io/0ovzivqyfai/_personal/icon/arrow-left-circle-fill_5dTlQgcEU.svg?ik-sdk-version=javascript-1.4.3&updatedAt=1652820112822';
  const rightArrowURL =
    'https://ik.imagekit.io/0ovzivqyfai/_personal/icon/arrow-right-circle-fill_SCGmR7MGA.svg?ik-sdk-version=javascript-1.4.3&updatedAt=1652820112843';
  ...
  const updateIndex = (mode) => {
    ...
    if (mode === 'inc') {
      setActiveIndex((newIndex) =>
        newIndex + 1 >= models.length ? models.length - 1 : newIndex + 1
      );
    } else if (mode === 'dec') {
      setActiveIndex((newIndex) => (newIndex - 1 < 0 ? 0 : newIndex - 1));
    }
  };
  ...
  return (
    <div className={`models-filter__container ${className}`}>
      <div className='models-filter__inner-container'>
        <div className='models-filter__models-container'>
          <h3 className='models-filter__title is-mobile'>
            Vyberte si modely <br /> ktoré sa hodia k Vášmu životnému štýlu
          </h3>

          <div
            className='models-filter__models-carousel'
            style={{ transform: `translateX(-${activeIndex * 100}%)` }}
          >
            {models.map((model, index) => (
              <div
                key={`model-${index}`}
                className={`models-filter__model ${activeIndex === index ? 'active' : ''}`}
                data-index={index}
              >
          ...
          <div className='models-filter__models-nav'>
            <span
              className={`icon left-nav ${activeIndex === 0 ? 'disabled' : ''}`}
              style={{
                WebkitMask: `url(${leftArrowURL}) no-repeat center`,
                mask: `url(${leftArrowURL}) no-repeat center`
              }}
              onClick={() => {
                updateIndex('dec');
              }}
            ...
            />
            <span
              className={`icon right-nav ${activeIndex === models.length - 1 ? 'disabled' : ''}`}
              style={{
                WebkitMask: `url(${rightArrowURL}) no-repeat center`,
          ...
          <div className='btn-wrapper is-mobile'>
            <button className='models-filter__models-show-filter' onClick={showFilter}>
              Filtre
            </button>
          </div>
        </div>
        ...
        <form
          className={`models-filter__filter-container ${
            isMobile ? (filterShown ? 'open' : 'closed') : ''
          }`}
        >
          <h3 className='models-filter__title'>
            Vyberte si modely <br /> ktoré sa hodia k Vášmu životnému štýlu:
          </h3>
